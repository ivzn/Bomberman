...
public class GridMap extends JPanel implements KeyListener
{
	...
	//Parameters: none
	//Return: N/A
	public GridMap (int settingLevel, int settingTime) {
		//t.start();
		addKeyListener(this);
		setFocusable(true);
	...
	public void paintComponent(Graphics g) {
		...
		if (drawcheck == 1) {
			...
			//For the entire map this adds the enemies
			for (int i = 1; i < 16; i++)
				for (int j = 1; j < 16; j++)
		...
		g.drawString("P1: " + p1.getPoints(), 120, windowLength + 35);

		g.setFont(new Font("Arial", Font.BOLD, 20));
		g.drawString("P2: " + p2.getPoints(), windowLength/2 + 100, windowLength + 35);
		...
		public void run ()
		{
			...
			while (timeInSec > 0)
			{
				if (timeInSec % 1000 == 0)//every second or 1000 milliseconds update the count down clock
				{
					...
					if(Enemy.getTotalNumEnemies() < Enemy.getMaxNumEnemies())
					{
						//For the entire map this adds the enemies
						for (int i = 1; i < 16; i++)
							for (int j = 1; j < 16; j++)
								if(locationsArray[i][j] == 0)//if the grid location is empty and there is less than the maximum amount of enemies, spawn one
									if(Math.random() > 0.992) //sometimes spawn
									{
										int oneOrTwo = (int)(Math.round(Math.random()+1));
										Enemy tempEnemy = new Enemy(i,j,oneOrTwo);
										locationsArray[i][j] = 3;
	...
	public JPanel getGameBoard() {
		panel = new GridMap(settingLevel, settingTime);
		return panel;
	}
